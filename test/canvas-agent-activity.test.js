...
test("PenEcho Agent activity is a removable user-only sibling outside capture and object state",()=>{
  ...
  assert.match(read("public/style.css"),/--studio-agent-glass:\s*color-mix\(in srgb, var\(--studio-panel\) 80%, transparent\)[\s\S]*?body\[data-theme="studio"\]\.studio-agent-docked \.canvas-agent-panel\s*\{[\s\S]*?background:\s*var\(--studio-agent-glass\)[\s\S]*?-webkit-backdrop-filter:\s*saturate\(1\.08\) blur\(30px\)[\s\S]*?backdrop-filter:\s*saturate\(1\.08\) blur\(30px\)/,"the activity contract stays tied to the actual Studio Agent sidebar material");
  ...
  assert.match(css,/\.canvas-agent-dialog-progress > i\s*\{[^}]*align-self:\s*start;[^}]*margin-top:\s*4\.5px;[^}]*background:\s*color-mix\(in srgb, var\(--studio-text, #1f2937\) 42%, transparent\);[^}]*box-shadow:\s*none;/,"ordinary dialog step markers are faint, halo-free, and centered on the title line");
  assert.match(css,/\.canvas-agent-dialog-progress small\s*\{[^}]*overflow:\s*visible;[^}]*text-overflow:\s*clip;[^}]*white-space:\s*normal;[^}]*overflow-wrap:\s*anywhere/ ,"visible progress explanations wrap instead of being truncated");
  assert.match(css,/:is\(#pe-type-contract, \.canvas-agent-dialog-progress\) strong\s*\{[^}]*font:\s*400 13px\/1\.25/ ,"generated activity titles stay regular");
  assert.match(css,/:is\(#pe-type-contract, \.canvas-agent-dialog-progress\) \.canvas-agent-dialog-progress-meta\s*\{[^}]*font:\s*400 11\.5px\/1\.3/ ,"generated activity metadata stays regular");
