...
test("active pen drawing paints a live layer before committing authoritative ink on release", () => {
  ...
  assert.match(activeDrawing, /normalizedDrawingSamples\(e\)[\s\S]*?for \(let index = 0; index < samples\.length; index\+\+\)[\s\S]*?const sample = samples\[index\][\s\S]*?drawingClientPoint\(d, sample\)[\s\S]*?appendLiveInkSample\(d, p, size\)[\s\S]*?commitLiveInkDrawingProgress\(d\)/);
