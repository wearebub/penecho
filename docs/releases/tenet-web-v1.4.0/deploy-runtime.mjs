...
const BACKUP = `/opt/tenet-demo/backups/whiteboard-1.4.0-${config.sourceCommit.slice(0,7)}`;
...
if(mode==='prepare') {
  ...
  assert.equal(manifest.tag,'tenet-web-v1.4.0'); assert.equal(manifest.sourceCommit,config.sourceCommit);
  const archiveName='Tenet-Whiteboard-runtime-1.4.0.tar';
  ...
  assert.equal(expected.size,27,'Unexpected runtime scope');
