...
const BACKUP = `/opt/tenet-demo/backups/whiteboard-1.7.2-${config.sourceCommit.slice(0,7)}`;
...
async function restore(receipt) {
  ...
  // This verified client-only overlay must not restart the demo host.
  for(const file of receipt.files) {
    if(file.sha256===file.previousSha256) continue;
  ...
  // Existing processes continue serving the atomically replaced client bundle.
  await ready();
  assert.equal(fileHash(POLICY),receipt.policyBeforeSha256,'Policy changed during client-only deployment');
  receipt.rollbackServed=await verifyServed(receipt,true);
  assert.equal(services(),receipt.servicesBefore,'Client-only rollback must preserve service PIDs');
...
if(mode==='prepare') {
  ...
  assert.equal(manifest.tag,'tenet-web-v1.7.2'); assert.equal(manifest.sourceCommit,config.sourceCommit);
  const archiveName='Tenet-Whiteboard-runtime-1.7.2.tar';
  ...
  assert.deepEqual(records.filter(file=>file.sha256!==file.previousSha256).map(file=>file.path).sort(), ['public/app.js','src/client/app/tenet-voice.js'].sort(), 'Only the voice source and client bundle may change');
  ...
  try {
    ...
    assert.equal(receipt.servicesAfter,receipt.servicesBefore,'Client-only publication must preserve service PIDs');
    receipt.status='installed'; receipt.installedAt=new Date().toISOString();
    receipt.scope='Client-only voice hotfix: changed public/app.js and src/client/app/tenet-voice.js only. All service PIDs and district policy bytes preserved; no Gateway restart, key rotation, notebook, dependency, server, authentication, or native binary changes.';
    writeJson(receiptPath,receipt);
