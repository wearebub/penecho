...
const ALLOWED_FIELDS = new Set(["eventId", "action", "modelName", "channel", "client", "platform", "appVersion"]);
...
function normalizeModelEvaluation(value) {
  ...
  if (!EVENT_ID_PATTERN.test(eventId) || !ACTIONS.has(action) || !SAFE_LABEL_PATTERN.test(modelName) || channel.length > 80 || !SAFE_LABEL_PATTERN.test(channel) || !CLIENTS.has(client) || !PLATFORMS.has(platform) || !VERSION_PATTERN.test(appVersion)) return null;
  return { eventId, action, modelName, channel, client, platform, appVersion };
}

async function forwardModelEvaluation(fetchImpl, cloudOrigin, event, timeoutMs = 10_000) {
  const target = new URL("/api/v1/model-evaluation", cloudOrigin);
  const response = await fetchImpl(target, {
    method:"POST",
    headers:{ accept:"application/json", "content-type":"application/json", "user-agent":"penecho-model-evaluation" },
